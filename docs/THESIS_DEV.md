...
또한 `frontend/SWYP_REVIEW.md`는 해당 시점의 회고 문서이며, 현재 서비스명(ScholarDot) 및 배포 URL 안내를 반영해 두었습니다.
...
- **처리 파이프라인**: PDF 파싱 → 문서·문단 구조 분석 → OpenAI 기반 문장 단위 번역·텍스트 처리 → 프론트에서 병렬 렌더링(원문/번역) 및 읽기 보조 UI.
- **구조적 관점**: 읽기 경험·상태 복원·인터랙션은 **프론트(Next.js)** 중심, 인증·파일·번역 파이프라인·구조 분석·복잡도 계산은 **백엔드(Spring Boot)** 에 두는 **관심사 분리** 및 프론트/백엔드 **분리 배포**.
...
| **구현** | 프론트/백엔드 핵심 기능별 구현 내용 (이미 README에 일부 있음) |
...
- [x] **시스템 구성도** 그리기 (사용자 → 프론트 → 백엔드 → DB/스토리지/OpenAI)
...
> 본 작업은 프론트 레이아웃 불안정성과 로컬 개발 환경 기동 오류를 해소하기 위한 것으로, CSS 모듈 전환과 환경 변수 정비를 통해 개발·시연 환경의 안정성을 높이는 효과를 얻었다.
...
- **`docs/ARCHITECTURE.md` 전면 재작성**
  ...
  - Mermaid 시스템 구성도: 프론트 → 백엔드 → DB/S3/OpenAI 전체 흐름 다이어그램.
...
- **색상 사전 선택 UI 추가 (프론트 - ReadList.tsx)**
...
- **복습 큐 → 문장 위치 점프 (프론트 - ReadList.tsx)**
...
- **완성된 형광펜 기능 전체 흐름**
  ...
  - 텍스트 선택 후 팝오버 "하이라이트" 클릭 시 백엔드 API(`POST /notes`) 저장도 병행.
...
- 형광펜 기능 구현 계획 (프론트 - @components/Read.tsx)
...
- **복습 큐 항목 삭제 버튼 추가 (프론트 - ReadList.tsx)**
...
- **번역 데이터 새로고침 내구성 개선 (프론트 - ReadList.tsx, NewDocument.tsx)**
...
- **카카오 로그인·CORS·OAuth (백엔드 분리 배포)**
  ...
  - CORS 및 로그인 후 리다이렉트: `SecurityConfig`가 `scholardot.frontend.base-url` 단일 출처 사용 → 프로덕션에서는 `FRONTEND_BASE_URL=https://scholardot.vercel.app` 등으로 맞출 것. Vercel `NEXT_PUBLIC_API_URL`은 배포된 백엔드 URL로 설정.
...
- **3색 하이라이트 색상 변경 + 토글 + 문서별 저장 (프론트 - ReadList.tsx)**
...
- **이어읽기 기능 추가 (프론트 - localStorage.ts + ReadList.tsx)**
...
- **복습 큐 기능 강화 (프론트 - ReadList.tsx + readList.module.css)**
...
- **mock 데이터 4페이지로 확장 (프론트 - mockTranslationData.ts)**
...
- **읽기 화면 + 사이드바 UI 개선 (프론트 - readList.module.css + ReadList.tsx)**
...
- **검색 이동 기능 추가 (프론트 - ReadList.tsx + readList.module.css)**
...
- **메모 툴팁 추가 (프론트 - ReadList.tsx + readList.module.css)**
...
- **메모 로컬 저장 기능 추가 (프론트 - ReadList.tsx)**
...
- **하이라이트 기본 색상 변경 (프론트 - ReadList.tsx + readList.module.css)**
...
- **원본 PDF 다운로드 API 추가 (백엔드 - DocumentController.java)**
...
- **파일 메타데이터 스키마 확장 (백엔드 - DocumentFile.java / DocumentFileService.java)**
...
- **파일 ID 기반 다운로드 API 추가 (백엔드 - DocumentController.java / DocumentDownloadService.java)**
...
- **배포 환경 변수 정리 (Vercel + Railway)**
  - Vercel 변수(`NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_BASE_URL`, OAuth redirect URI들)와 Railway 변수(`FRONTEND_BASE_URL`, `SPRING_DATASOURCE_*`, `KAKAO_*`, `JWT_SECRET`, `UPLOAD_DIR`)를 실서비스 도메인 기준으로 정리.
  - 운영 도메인 기준: 프론트 `https://scholardot.vercel.app`, 백엔드 `https://scholardot-production.up.railway.app`.
  - Kakao Redirect URI는 프론트가 아니라 백엔드 콜백(`.../login/oauth2/code/kakao`)으로 맞추는 규칙을 확정.

- **카카오 로그인 운영 점검**
  - 구조 점검 결과: 카카오 로그인은 프론트 버튼이 백엔드 OAuth 엔드포인트로 이동하는 방식이므로 백엔드 공개 HTTPS 배포가 필수.
  - CORS/리다이렉트 기준이 `scholardot.frontend.base-url` 단일 값이므로, 운영 시 `FRONTEND_BASE_URL=https://scholardot.vercel.app`로 고정.
...
- **SecurityConfig permitAll 범위 확장 (백엔드 - SecurityConfig.java)** _(2026-04-02)_
  ...
  - 관련 파일: `backend/app/scholardot/src/main/java/swyp/scholardot/common/SecurityConfig.java`
...
### 2026-04-02 (프론트 마이페이지 UI 완성도 고도화 + 카카오 로그인 운영 점검)

- **내 계정 페이지 카드형 리디자인 (프론트 - `app/mypage/account/page.tsx`, `account.module.css`)**
...
- **내 문서함 empty state 개선 (프론트 - `app/mypage/mydocument/page.tsx`, `document.module.css`)**
...
- **회원 탈퇴 모달 UI 고도화 (프론트 - `app/components/modal/DeleteUserModal.tsx`, `DeleteUserModal.module.css`)**
...
- **카카오 로그인 운영 오류 단계별 점검**
  ...
  - 로그인 완료 후 구 도메인(`paperdot`)으로 이동하는 증상은 BE의 `FRONTEND_BASE_URL` 적용 상태 점검 필요로 정리.
...
- **원인 2: 프론트 ownerId null 전송 (레이스 컨디션)**
  ...
  - 조치 (프론트): `accessToken`이 없으면 업로드 시도 차단, `ownerId` 폼 데이터 제거.
  - 조치 (백엔드): `DocumentController`에서 `ownerId`를 프론트 폼이 아닌 JWT 토큰(`SecurityContextHolder`)에서 직접 추출.
...
### 2026-04-03 — ScholarDot MVP 구현 완료 🎓

> **2025년 말부터 진행한 설계·구현 과정을 거쳐 핵심 기능을 모두 구현한 MVP를 완성하였다.**
> 이후에는 성능 개선, 사용자 평가, 논문 작성 및 유지보수를 중심으로 프로젝트를 고도화한다.

---

#### 오늘 마지막으로 완료한 작업

- **읽기 화면 사이드바 PDF 썸네일 표시 수정 (프론트 - ReadList.tsx)**
...
- **문서함 PDF 즉시 표시 구조 완성 (프론트 - mydocument/page.tsx)**
...
- **문서 삭제 기능 추가 (프론트 + 백엔드 - mydocument/page.tsx, api/document.ts)**
...
- **홈 화면 Demo 섹션 YouTube 영상 삽입 (프론트 - page.tsx)**
...
- **"어떻게 동작하나요?" 모달 이미지 교체 (프론트 - page.tsx)**
...
| **배포 URL** | 프론트: `https://scholardot.vercel.app` · 백엔드: `https://scholardot-production.up.railway.app` |
...
3. **형광펜 3색 하이라이트 + 메모 시스템** — 문장 클릭으로 파랑/초록/분홍 3색 하이라이트 토글. 텍스트 선택 후 팝오버로 하이라이트/메모 저장. localStorage 즉시 반영 + 백엔드 API 비동기 영속화 이중 저장.
...
8. **프로덕션 풀스택 배포** — Vercel(프론트) + Railway(백엔드 + PostgreSQL) 운영 환경 구성. 환경 변수 분리, CORS·Security 설정, 스토리지 fallback(S3/로컬) 처리.
...
### 2026-04-03 (학술 논문 전용 번역 함수 구현 — 백엔드)

- **`AcademicTranslationItem` DTO 추가 (백엔드 - `OpenAiTranslationDto.java`)**
...
- **`translateAcademic()` 메서드 구현 (백엔드 - `OpenAiTranslator.java`)**
...
- **외부 API 불안정성 대응: 재시도 로직 (백엔드 - `OpenAiTranslator.java`)**
...
- **번역 결과 신뢰성 검증: 문장 수 검증 (백엔드 - `OpenAiTranslator.java`)**
...
- **읽기 진행 표시·이어 읽기 정확도 (프론트 — `ReadHeader.tsx`, `readHeader.module.css`, `ReadList.tsx`, `lib/localStorage.ts`)**
...
- **새로고침 후 로그인·데모 세션 (프론트 — `lib/authSession.ts`, `AuthBootstrap.tsx`, `Layout.tsx`, `IsLogin.tsx`, `HeaderModal.tsx`, `login/page.tsx`, `account/page.tsx`, `useLogin.ts`, `ReadList.tsx`)**
...
- **문서 라이브러리 UI (읽기 연속성·발견성 중심) (프론트 — `useDocumentLibrary.ts`, `DocumentLibraryCard.tsx`, `DocumentPdfModal.tsx`, `mydocument/page.tsx`, CSS 모듈)**
...
- **메모: 빈 내용 저장 시 삭제 (프론트 — `ReadList.tsx`)**
...
- **작업 전환 및 개발 체크리스트 (우선순위 재정의)**
  ...
  - **오늘 작업 체크리스트**
    ...
    - [x] 분석 결과 타입/인터페이스 정리 (프론트 표시용 공통 스키마 확정)
...
- **완료한 구현 항목**
  ...
  - **문서 구조 분석 v1 (백엔드)**: `DocumentStructureAnalysisService` + `DocumentStructureAnalysisResponse` / `PageStructureStats` DTO 구현.  
...
- **체크리스트 진행 상태 (4/10 오후 기준)**
  ...
  - [x] 분석 결과 타입/인터페이스 정리 (프론트 공통 스키마)
  - [x] 문서 분석 요약 UI 추가 (카드형 지표 + 핵심 수치)
...
### 2026-06-16 (프론트·백엔드 — 체험 모드 UX, 더미데이터·배포 수정, PDF 텍스트 정규화)

- **배경**: 시연용 `sample_test.pdf` 더미데이터 개편·배포 반영 과정에서 Vercel 빌드 실패, 체험 모드와 실제 로그인 기능 경계 미정리, PDF 추출 텍스트의 비정상 띄어쓰기 이슈가 겹쳐 정리.

#### 더미데이터·시연 읽기 (`mockTranslationData.ts`)

- `sample_test.pdf` 체험용 mock: 양자물리 블록 제거, 5페이지 구성 단순화.
- `MixedTextWithMath`에서 줄바꿈·분할로 `$...$` / `$$...$$`가 깨지던 문제 → 페이지 5는 산문 + display 수식 1문장(EM·F1)만 유지, 인라인 수식은 prose로 단순화.
- 체험 모드는 `documentId` 없이 `MOCK_TRANSLATION_PAIRS` 사용(`ReadList`); 데모 진입 시 `clearStoredDocumentSessionForDemo()`로 실제 문서 세션과 분리.

#### PDF 텍스트 띄어쓰기 정규화

- **프론트**: `frontend/app/lib/pdfTextNormalize.ts` — `PD F`→`PDF`, `Scholar Dot`→`ScholarDot`, `Sentence- BERT` 등 사전 치환 + 하이픈·약어 패턴. `MixedTextWithMath` 표시 전후 적용.
- **백엔드**: `PdfExtractTextNormalizer.java` — `splitToSentences` 직전 동일 계열 보정, `PdfExtractTextNormalizerTest` 단위 테스트.

#### Vercel 배포 빌드 수정

- `MixedTextWithMath.tsx`: 반환 타입 `ReactNode` import 누락 → `import { useMemo, type ReactNode } from "react"`.
- `pdfTextNormalize.ts`: `.sort()` 후 튜플 타입 widen → `as [string, string][]`로 TS 오류 해결.
- 위 오류로 프로덕션 빌드가 중단되어 더미데이터 변경이 배포에 반영되지 않던 원인 제거.
...
- **`isDemoUserActive()`** (`lib/authSession.ts`): `demo-user` 또는 `scholardot-session-mode=demo` 판별 공통화.
- **헤더 메뉴** (`HeaderModal.tsx`)
  - 「내 문서함」「내 계정」「새 문서 만들기」: 체험 시 토스트 *「로그인 후에만 이용할 수 있습니다.」* → `/login`.
  - **로그아웃**: 체험 세션 `clearDemoSession()` + 스토어 초기화 후 **`window.location.href = "/"`** (홈). `/read`에서 `router.push`만 쓰면 Layout 보호 라우트 가드가 먼저 `/login`으로 보내는 레이스가 있어 하드 이동으로 해결.
- **URL 직접 접근 가드** (`Layout.tsx`): `/mypage/*`, `/newdocument` — 체험 유저 동일 토스트 후 `/login`으로 `replace`.

#### 관련 파일 (요약)

| 구분 | 경로 |
|------|------|
| 더미·체험 읽기 | `mockTranslationData.ts`, `ReadList.tsx`, `authSession.ts` |
| 수식·텍스트 | `MixedTextWithMath.tsx`, `pdfTextNormalize.ts` |
| 요약 UI | `DocumentContentSummaryPanel.tsx` |
| 체험 가드 | `HeaderModal.tsx`, `Layout.tsx` |
| 백엔드 정규화 | `PdfExtractTextNormalizer.java` |

> 논문 「구현·운영」: **게스트 체험(demo)** 과 **인증 사용자 기능**(문서함·업로드·LLM 요약)을 UI·라우트·세션 계층에서 분리한 사례. 시연은 mock·로컬 상태로, 유료/개인 데이터 경로는 OAuth·`documentId`로만 접근하도록 경계를 명시.

> 결과: 배포 가능 상태 복구, 시연 PDF·수식 표시 안정화, 체험 사용자의 오해(요약·문서함 미제공) 최소화 및 로그인 유도 UX 정리, 체험 로그아웃 시 비로그인 홈 복귀.

#### 백엔드·프론트 명칭 통일 (`paperdot` / `paper-dot` → `scholardot`)

- Gradle 프로젝트 경로: `backend/app/scholardot` (구 `app/paperdot` 중복 디렉터리 제거).
- Java 패키지: `swyp.paperdot.*` → `swyp.scholardot.*`, 메인 클래스 `ScholardotApplication`.
- 설정 prefix: `paperdot.*` → `scholardot.*` (`application.yml` / `application.properties`).
- JWT 리프레시 쿠키 기본명: `scholardot_refresh` (구 `paperdot_refresh`).
