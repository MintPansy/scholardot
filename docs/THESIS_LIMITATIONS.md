...
## 6. 논문 학습 흐름의 한계 및 시각 자료 탐색 개선

### 6.1 현재 시스템의 한계

현재 ScholarDot은 PDF를 문장 단위로 구조화하고,
번역 진행 상태와 완료된 문장을 점진적으로 제공한다.
사용자는 번역 완료 전에도 처리 상황을 확인하고,
완료된 문장부터 병렬 읽기를 시작할 수 있다.

그러나 논문 본문에서 Figure, Table, 그래프, 이미지를 확인하려는 경우
사용자는 원문 PDF를 별도 모달로 열고 해당 페이지를 직접 탐색해야 한다.
원문 확인 기능은 제공되지만, 본문을 읽던 위치에서 이탈하고
시각 자료의 위치를 다시 찾아야 하므로 읽기 맥락이 끊길 수 있다.

### 6.2 개선 목표

번역 처리 속도는 PDF 길이, 텍스트 추출 품질, LLM API 응답 시간에 영향을 받으므로
단기간에 대폭 개선하기 어려운 제약이 있다.
이에 본 연구에서는 대기 시간을 단순히 줄이는 접근과 함께,
번역 처리 중에도 사용자가 논문 구조와 핵심 근거를 탐색할 수 있게 하는
즉시 탐색 경험을 개선 목표로 설정한다.

### 6.3 개선 설계

- PDF에서 Figure/Table 캡션, 번호, 페이지 정보를 추출한다.
- 본문의 `Figure 1`, `Fig. 2`, `Table 3` 등 참조 표현을 식별한다.
- 본문 참조와 대응하는 시각 자료를 연결한다.
- 사용자가 참조를 클릭하면 캡션, 페이지 번호, 페이지 미리보기 또는 원문 페이지 이동을 제공한다.
- 초기 버전에서는 시각 자료 영역의 완벽한 자동 크롭보다,
  안정적인 원문 페이지 연결을 우선한다.

### 6.4 예상 효과

- 번역이 진행되는 동안에도 Figure/Table을 중심으로 논문의 핵심 결과를 탐색할 수 있다.
- 번역문과 원문 PDF 사이를 왕복하며 발생하는 탐색 시간을 줄일 수 있다.
- 본문의 주장과 그래프·표라는 근거를 연결해 학습 맥락을 유지할 수 있다.
- 향후 캡션 기반 검색, 그림·표 갤러리, 시각 자료와 관련 문단을 활용한 AI 질의 기능으로 확장할 수 있다.

### 6.5 평가 방법 및 한계

평가 시에는 Figure/Table 참조 클릭률, 원문 페이지 이동률,
미리보기 이후 본문 읽기 지속 여부, 과업 기반 사용성 테스트의 정성 피드백을 활용한다.

다만 PDF의 다단 편집, 복수 패널 Figure, 표 내부 구조, 스캔 PDF,
캡션의 비표준 형식에 따라 자동 매칭 정확도가 낮아질 수 있다.
따라서 잘못된 시각 자료 연결을 최소화하기 위해,
매칭 신뢰도가 낮은 경우에는 자동 미리보기 대신 원문 페이지 이동을 제공하는
보수적 처리 전략이 필요하다.
