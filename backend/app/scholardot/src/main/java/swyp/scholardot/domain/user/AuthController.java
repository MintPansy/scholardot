...
public class AuthController {
    ...
    @Value("${scholardot.jwt.refresh-cookie-name}")
...
//            @CookieValue(name = "${scholardot.jwt.refresh-cookie-name}", required = false) String refreshToken
            @CookieValue(name = "scholardot_refresh", required = false) String refreshToken
    ...
    private Long resolveUserId(HttpServletRequest request) {
        ...
        if (auth != null && auth.getPrincipal() instanceof JwtAuthFilter.ScholardotPrincipal p) {
            return p.userId();
        }
